...
public:
	...
	void map_Render_Datamanager_Loop(HDC getMemDC);													// 맵을 그려주고, 루프시켜준다. (매개변수 : 속도값이 들어있는 배열 5개)
	...
	// 스컬이 이벤트 지역과 충돌 했다면
	void skul_Collision_Event(RECT eventRC);
	...
	// 이미지 벡터 삭제
	void Delete_ImageMaker() { return _ImageMake->release(); }
